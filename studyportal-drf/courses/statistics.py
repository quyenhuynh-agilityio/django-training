...
def get_statistics_cache_timeout():
    ...
    """
    return getattr(settings, 'STATISTICS_CACHE_TIMEOUT', 30)
...
class CourseStatistics:
    ...
    def _get_cache_key(cls, stat_type, **kwargs):
        ...
        return (
            f'{cls.CACHE_KEY_PREFIX}_{stat_type}_{params}'
            if params
            else f'{cls.CACHE_KEY_PREFIX}_{stat_type}'
        )
    ...
    def get_average_enrollments(cls, time_period=None):
        ...
        # Cache the result
        cache.set(cache_key, result, get_statistics_cache_timeout())
        return result
    ...
    def get_top_courses(cls, limit=10, time_period=None):
        ...
        cache.set(cache_key, result_list, get_statistics_cache_timeout())
        return result_list
    ...
    def clear_all_cache(cls):
        ...
        cache.delete_many(
            [
                cls._get_cache_key('avg_enrollments'),
                cls._get_cache_key('top_courses'),
                cls._get_cache_key('enrollment_trends'),
            ]
        )
